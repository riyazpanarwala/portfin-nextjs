 ...
 */
export const POST = withErrorHandler('POST /api/prices', async (request) => {
 ...
 */
export const PATCH = withErrorHandler('PATCH /api/prices', async (request) => {
  const { symbol, price } = await request.json();
  if (!symbol || price == null) return badRequest('symbol and price required');
